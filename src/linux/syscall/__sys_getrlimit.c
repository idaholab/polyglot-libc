...
/* XXX: getrlimit() is capped at 32-bit values, regardless of storage size */

int __sys_getrlimit(int resource, struct rlimit *rlim)
{
    struct __kernel_rlimit k_rlim;

    if (syscall(__NR_getrlimit, resource, &k_rlim) == -1)
        return -1;
